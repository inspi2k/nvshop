...
# 1-1. 입력값 받기 (mid / storename, title)
# 입력받은 storename - 네이버 쇼핑에서 검색했을 때 상품이 노출되는 스토어명
# 입력받은 title - 검색 결과에서 구분 가능한 상품명 일부 또는 전체
...
if check_mid == "OK":
    ...
    while (got_it == False) and (pageIndex <= 100):
        headers = {
        ...
        if items["shoppingResult"]["total"] > 4000:
            print("Too many items({}) in '{}'".format(items["shoppingResult"]["total"], get_store))
            pyautogui.alert(result + "\n\n" + "Too many items. {} is over 4000.\nRetry other query(storename+somewords).".format(items["shoppingResult"]["total"]))
            sys.exit(1)

        if items["shoppingResult"]["total"] < 1:
            get_mid = ""
            print("can not searching")
            pyautogui.alert(result + "\n\n" + "can not searching")
            sys.exit(0)
        else:
            # 1-3. MID값 찾기
...
get_write = pyautogui.prompt(
    "찾은 MID와 키워드를 시트에 기록하시겠습니까?\n(1:기록 / 0:무기록)\n\n(list에 이미 기록되어 있으면 pass, rank는 현재 시간으로 기록됨)", "상품 순위 찾기 (Step 3-1)", default=1
)
if get_write == "1":
    msg = "시트에 기록합니다\n\n"
else:
...
get_keyword = pyautogui.prompt(
    msg+"네이버 쇼핑에서 검색할 키워드를 입력하세요", "상품 순위 찾기 (Step 3-2)", default=default_keyword
)
...
if get_write == "1":
    try:
        ...
        if get_write_match == False:
            ...
            list_row.append('')
            list_row.append(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
if get_write == "1":
    ...
    warnings.filterwarnings(action="ignore")
    if 'r' in globals():
        worksheet.update('H' + str(r) + ':I' + str(r), [[opendate, product_rank]])
        worksheet.update('B' + str(r), get_store)
